...
class Carrito:
    ...
    def vaciar_carrito(self):
        self.cursor.execute("DELETE FROM carrito",)
        self.conexion.commit()
        return jsonify({'message': 'El carrito se vacio completamente.'}), 200
...
# 10 - Ruta para comprar productos
# DELETE: permite eliminar información.
@app.route('/carrito', methods=['DELETE'])
def comprar_productos():
    return carrito.vaciar_carrito()

# 11 - Ruta para obtener el index
@app.route('/')
def index():
